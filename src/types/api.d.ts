...
export interface paths {
    ...
    "/geo/directions": {
         ...
         *
         *     The shape is normalised: distance in metres, duration in seconds, a GeoJSON LineString and plain-text steps. No vendor field reaches the client, so changing provider is a config branch rather than a client rewrite. `isFallbackProvider` is true when the keyless OpenStreetMap router answered rather than the metered one — a deployment may choose it (`ROUTING_PROVIDER=osrm`), so clients must not read it as "development".
         */
        get: {
            parameters: {
                query: {
